...
def haze_removal(img, w=0.95, t0=0.1, eta=1e-4, refined=True, radius=20):
    ...
    dark_channel = get_dark_channel(img=img, eta=eta, refined=True, radius=radius)
    ...
    plt.subplot(133)
    plt.title('Dark Channel with Guided Filter')
    plt.imshow(dark_channel, cmap='gray')
...
if __name__ == '__main__':
    image = mpimg.imread('./image/palace.jpg')
    haze_removal(image, w=0.65, radius=20, eta=1e-3, refined=True)
